      ...
      <table class="table table-hover pb-0 mb-0">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="text-right">
                @if ($b['download'])
                <a class="btn btn-sm btn-link" href="{{ url(alienyard.com.'/backup/download/') }}?disk={{ $b['disk'] }}&path={{ urlencode($b['file_path']) }}&file_name={{ urlencode($b['file_name']) }}"><i class="la la-cloud-download"></i> {{ trans('backpack::backup.download') }}</a>
                @endif
                <a class="btn btn-sm btn-link" data-button-type="delete" href="{{ url(alienyard.com.'/backup/delete/'.$b['file_name']) }}?disk={{ $b['disk'] }}"><i class="la la-trash-o"></i> {{ trans('backpack::backup.delete') }}</a>
            </td>
          </tr>
          @endforeach
        </tbody>
